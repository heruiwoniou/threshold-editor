...
const TimeLineConstructor = ViewModel.extend({
  tpl(props, state, parentState) {
    ...
    return (
      <div
        className="threshold-editor__timeline"
              ...
              return (
                <div
                  className={
                    ...
                    colors[(list.length - 1 - i) % colors.length] +
                    (editing ? " is-editing" : "")
                  }
                  style={{ zIndex }}
                  id={list.length - 1 - i % colors.length}
                >
                  <div
                    className="threshold-editor__timebar__child"
                    style={{ left: 0, right: to }}
                  />
                  <div
                    className="threshold-editor__timebar__child"
                    style={{ right: 0, left: from }}
                  />
                </div>
              );
            } else {
